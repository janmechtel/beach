"""
compare.py — compare a candidate action JSON against the manual ground truth.

Use via the Click command function `compare_cmd`.
...
def compare_cmd(candidate: Path, ref: Path, tol: float) -> None:
    """Compare CANDIDATE action JSON against a reference ground truth."""
    try:
        compare(ref, candidate, tol)
